...
"
" CONSIDER TODO: Make a list of exclude keys, and map them so that they set w:Smooth_Ignore_Next_Movement.  For example this could apply to `/` and `?` with 'hlsearch' enabled, and maybe also to `d`.
"
...
if !exists("g:Smooth_Enabled")
  let g:Smooth_Enabled = 1
endif

" We can only really see the cursor moving if 'cursorline' is enabled
if !exists("g:Smooth_CursorTime")
  let g:Smooth_CursorTime = ( &cursorline || exists("g:hiline") && g:hiline ? 5 : 0 )
endif

" By default, scrolling the buffer is slower then moving the cursor, because a page of text is "heavier" than the cursor.  :)
if !exists("g:Smooth_ScrollTime")
  let g:Smooth_ScrollTime = 10
endif

if !exists("g:Smooth_MaxTime")
  let g:Smooth_MaxTime = 500
endif

if !exists("g:Smooth_EasingStyle")
  let g:Smooth_EasingStyle = 2
endif

if !exists("g:Smooth_DetectPendingKeys")
  let g:Smooth_DetectPendingKeys = 1
endif

if !exists("g:Smooth_Debug")
  let g:Smooth_Debug = 0
endif
...
command! SmoothToggle call s:ToggleEnabled()
...
function! s:CheckForChange(actIfChange)
  ...
  if a:actIfChange && g:Smooth_Enabled
...
function! s:smooth_scroll(start, end)
  ...
  let timeForCursorMove = g:Smooth_CursorTime * s:hypot(numLinesToTravel, numColumnsToTravel)
  let timeForScroll = g:Smooth_ScrollTime * s:hypot(numLinesToScroll, numColumnsToScroll)
  ...
  let totalTime = 1.0 * min([g:Smooth_MaxTime,max([0,totalTime])])
  ...
  if exists("w:interruptedAnimationAt")
    ...
    if timeSinceInterruption < 50
      ...
      "let totalTime = 1.0 * min([g:Smooth_MaxTime,float2nr(totalTime + w:interruptedAnimationTimeRemaining)])
    endif
  ...
  while 1
    ...
    if g:Smooth_DetectPendingKeys && getchar(1)
...
function! s:ToggleEnabled()
  let g:Smooth_Enabled = !g:Smooth_Enabled
  echo "Smooth is " . ( g:Smooth_Enabled ? "en" : "dis" ) . "abled"
endfunction
